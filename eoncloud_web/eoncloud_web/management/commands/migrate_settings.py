...
class Command(BaseCommand):
    option_list = BaseCommand.option_list + (
        ...
        optparse.make_option(
            ...
            dest='listdiff',
            default=False,
            help=('List diff of configuration names between local_settings.py '
                  'and local_settings.py.example'),
        ),
        optparse.make_option(
            '-f', '--force',
            action='store_true',
    ...
    def diff_options(self):
        ...
        with DirContext(self.local_settings_dir) as dircontext:
            ...
            example_settings = imp.load_source('example_settings',
                                               self.local_settings_example)

            local_settings = imp.load_source('local_settings',
                                             self.local_settings_file)

            example_settings_set = set(dir(example_settings))
            local_settings_set = set(dir(local_settings))

            diff_set = example_settings_set - local_settings_set

            if not diff_set:
                print("No missing configurations")
                return
    ...
    def handle(self, *args, **options):
        ...
        if options.get('listdiff'):
            self.diff_options()
        elif options.get('gendiff'):
            self.gendiff(force)
        else:
            self.patch(force)
